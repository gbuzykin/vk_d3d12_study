...
struct MainWindow::ImplData {
    ...
    xcb_window_t window{};
    xcb_intern_atom_reply_t* delete_window_atom = nullptr;
...
void MainWindow::showWindow() {
    xcb_intern_atom_reply_t* protocols_atom = ::xcb_intern_atom_reply(
        data_->connection, ::xcb_intern_atom(data_->connection, 1, 12, "WM_PROTOCOLS"), 0);

    data_->delete_window_atom = ::xcb_intern_atom_reply(
        data_->connection, ::xcb_intern_atom(data_->connection, 0, 16, "WM_DELETE_WINDOW"), 0);
            ...
            for (std::uint32_t n = 0; n < data_->key_state_table.size(); ++n) {
                ...
                if (key_state.release) {
                    if (std::chrono::duration<double>(timer_now - key_state.release_timer_start).count() > 0.01) {
                        ...
                        if (!onKeyEvent(static_cast<KeyCode>(n), n, false, ret_code)) { return ret_code; }
                    } else {
                        has_pressed = true;
                    }
                }
            ...
            switch (event->response_type & ~0x80) {
                ...
                case XCB_KEY_PRESS: {
                    ...
                    if (ev->detail >= 1 && ev->detail < data_->key_state_table.size()) {
                        ...
                        if (!key_state.release) {
                            if (!onKeyEvent(static_cast<KeyCode>(ev->detail), ev->detail, true, ret_code)) {
                                return ret_code;
                            }
                        }
                        key_state.release = false;
