...
bool RenderTarget::create(const uxs::db::value& create_info) {
    // Drawing synchronization

    if (!device_.createSemaphore(sem_image_acquired_)) { return false; }
    if (!device_.createSemaphore(sem_ready_to_present_)) { return false; }
    ...
    VkResult result = vkCreateRenderPass(~device_, &render_pass_create_info, nullptr, &render_pass_);
...
ResultCode RenderTarget::beginRenderTarget(const Vec4f& clear_color) {
    if (!device_.waitForFences(std::array{fence_drawing_}, false, 5000000000)) { return ResultCode::FAILED; }

    std::uint32_t image_index = 0;
    if (const auto result = swap_chain_.acquireImage(2000000000, sem_image_acquired_, VK_NULL_HANDLE, image_index);
...
ResultCode RenderTarget::endRenderTarget() {
    auto& graphics_queue = device_.getGraphicsQueue();
    auto& present_queue = device_.getPresentQueue();

    if (current_image_index_ == INVALID_UINT32_VALUE) { return ResultCode::FAILED; }
    ...
    if (!command_buffer_.endCommandBuffer()) { return ResultCode::FAILED; }

    if (!device_.resetFences(std::array{fence_drawing_})) { return ResultCode::FAILED; }
    ...
    return present_queue.presentImages(std::array{sem_ready_to_present_},
...
void RenderTarget::bindVertexBuffer(std::uint32_t first_binding, IBuffer& buffer, std::size_t offset) {
    command_buffer_.bindVertexBuffers(first_binding, {std::array{~static_cast<Buffer&>(buffer)}, std::array{offset}});
}
