...
namespace {
constexpr std::array g_instance_extensions{
...
bool RenderingDriver::createInstance(const uxs::db::value& app_info) {
    ...
    const VkInstanceCreateInfo create_info{
        ...
        .enabledExtensionCount = std::uint32_t(g_instance_extensions.size()),
        .ppEnabledExtensionNames = g_instance_extensions.data(),
    };
    ...
    const auto is_extension_enabled = [](const char* extension) {
        return std::ranges::any_of(g_instance_extensions, [extension](const char* enabled_extension) {
