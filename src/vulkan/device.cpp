...
bool Device::writeToDeviceLocalMemory(VkDeviceSize data_size, const void* data, VkBuffer dst, VkDeviceSize dst_offset,
    ...
    if (!writeToHostVisibleMemory(staging_buffer.getMemoryObjectHandle(), 0, data_size, data)) { return false; }
...
IShaderModule* Device::createShaderModule(std::span<const std::uint8_t> source_spirv,
                                          const uxs::db::value& create_info) {
    auto shader_module = std::make_unique<ShaderModule>(*this);
    if (!shader_module->create(source_spirv, create_info)) { return nullptr; }
...
IPipeline* Device::createPipeline(IRenderTarget& render_target, std::span<IShaderModule* const> shader_modules,
    ...
    for (std::size_t n = 0; n < shader_modules.size(); ++n) {
        shader_stage_create_infos.emplace_back(Wrapper<VkPipelineShaderStageCreateInfo>::unwrap({
            .shader_stage = stages[n],
            .shader_module = ~static_cast<ShaderModule&>(*shader_modules[n]),
            .entry_point_name = "main",
        }));
    }

    auto pipeline = std::make_unique<Pipeline>(*this);
    if (!pipeline->create(static_cast<RenderTarget&>(render_target), shader_stage_create_infos, create_info)) {
        return nullptr;
    }
