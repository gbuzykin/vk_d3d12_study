...
Device::~Device() {
    ...
    ObjectDestroyer<VkShaderModule>::destroy(device_, pixel_shader_module_);
...
ISwapChain* Device::createSwapChain(ISurface& surface, const uxs::db::value& opts) {
    auto swap_chain = std::make_unique<SwapChain>(*this, static_cast<Surface&>(surface));
    if (!swap_chain->create(opts)) { return nullptr; }
    return swap_chains_.emplace_back(std::move(swap_chain)).get();
}
...
bool Device::prepareTestScene(ISurface& surface) {
    ...
    std::vector<std::uint8_t> vertex_shader_spv;
    if (uxs::bfilebuf ifile("data/shaders/simple/vert.spv", "r"); ifile) {
        vertex_shader_spv.resize(ifile.seek(0, uxs::seekdir::end));
        ifile.seek(0);
        ifile.read(vertex_shader_spv);
    }

    std::vector<std::uint8_t> pixel_shader_spv;
    if (uxs::bfilebuf ifile("data/shaders/simple/pix.spv", "r"); ifile) {
        pixel_shader_spv.resize(ifile.seek(0, uxs::seekdir::end));
        ifile.seek(0);
        ifile.read(pixel_shader_spv);
    }

    if (!createShaderModule(vertex_shader_spv, vertex_shader_module_)) { return false; }
    if (!createShaderModule(pixel_shader_spv, pixel_shader_module_)) { return false; }
...
RenderTargetResult Device::renderTestScene(ISwapChain& swap_chain) {
    auto& swap_chain_impl = static_cast<SwapChain&>(swap_chain);

    std::uint32_t image_index = 0;
    if (const auto render_target_result = swap_chain_impl.acquireImage(2000000000, sem_image_acquired_, VK_NULL_HANDLE,
                                                                       image_index);
        render_target_result != RenderTargetResult::SUCCESS) {
    ...
    if (!command_buffer_.endCommandBuffer()) { return RenderTargetResult::FAILED; }

    if (!resetFences(std::array{fence_drawing_})) { return RenderTargetResult::FAILED; }
    ...
    if (const auto render_target_result = present_queue_.presentImages(
            std::array{sem_ready_to_present_[n_frame_]}, {std::array{~swap_chain_impl}, std::array{image_index}});
        render_target_result != RenderTargetResult::SUCCESS) {
        return render_target_result;
    }

    if (!waitForFences(std::array{fence_drawing_}, VK_FALSE, 5000000000)) { return RenderTargetResult::FAILED; }
...
bool Device::createGraphicsPipelines(util::multispan<const VkGraphicsPipelineCreateInfo, VkPipeline> pipelines,
...
bool Device::writeToDeviceLocalMemory(DevQueue& queue, CommandBuffer& command_buffer, VkDeviceSize data_size,
    ...
    if (!createBuffer((data_size + non_coherent_atom_size - 1) & ~(non_coherent_atom_size - 1),
