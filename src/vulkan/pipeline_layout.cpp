...
bool PipelineLayout::create(const uxs::db::value& config) {
    uxs::inline_dynarray<VkDescriptorSetLayoutBinding> ds_layout_bindings;

    ds_layout_bindings.push_back(VkDescriptorSetLayoutBinding{
        .binding = 0,
    ...
    const VkDescriptorSetLayoutCreateInfo ds_create_info{
        ...
        .bindingCount = std::uint32_t(ds_layout_bindings.size()),
        .pBindings = ds_layout_bindings.data(),
    };

    VkResult result = vkCreateDescriptorSetLayout(~device_, &ds_create_info, nullptr, &descriptor_set_layout_);
